...
verticals, which wrote to a registry rather than to anyone. `bin/verify.cljk`
...
`bin/verify.cljk` refuses. A 40-URL random sample checked
...
Step by step from a fresh clone, including fetching from B2:
[`docs/operator-quickstart.md`](docs/operator-quickstart.md).

```bash
kbb --backend sci --classpath src:test test/gtm_data_verify_test.cljk   # checks, on fixtures
kbb --backend sci --classpath src bin/verify.cljk .                     # checks, on this archive
```
